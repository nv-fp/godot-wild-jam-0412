...
var angles = [-90, -50, 0, 50, 90, 140, 180, -140]
...
func _physics_process(_delta) -> void:
	if $RayCast2D.is_colliding():
		...
		if collider is TileMap:
			...
			var filtered = tilemap.counters.filter(func (counter): return counter.tile == tile)
			if filtered.size() > 0:
				interactable = filtered.front()
...
func unequip_item() -> Node2D:
	return remove_held_item()
	
func equip_item(item: StringName):
	spawn_in_held_item(str(item))
